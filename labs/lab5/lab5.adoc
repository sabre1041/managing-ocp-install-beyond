...
Take note of the secret that the binding operation created. Click the **Close** button on the confirmation page.
...
In the upper right hand corner, click **Add to Application**. From the dropdown select **mediawiki123**, add the secret as an **Environment Variable**. Click **Save**.
...
That will trigger a new deployment and the application will be ready after the deployment is finished.
