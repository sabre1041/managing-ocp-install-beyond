...
Once the pod has a status of _Success_, navigate to the **Overview** page.
...
However, since MediaWiki makes use of persistent storage using EBS volumes, only a single application instance can mount the volume at a time. To work around this issue, first scale down the MediaWiki application by clicking the arrow to expand the options.

image::images/mediawiki-apb-scale-arrow.png[]

Then scale the application down by clicking the down arrow. Then click _Scale Down_. 
